
package com.jyw.classinfo.mapper;

import com.jyw.model.SyllabusInfo;
import com.jyw.model.SyllabusInfoCriteria;
import java.util.List;
import org.apache.ibatis.annotations.Param;

public interface SyllabusInfoMapper {
    /** 
     * 根据指定的条件获取数据库记录数
     * @param example
     */
    long countByExample(SyllabusInfoCriteria example);

    /** 
     * 根据指定的条件删除数据库符合条件的记录
     * @param example
     */
    int deleteByExample(SyllabusInfoCriteria example);

    /** 
     * 根据主键删除数据库的记录
     * @param syllabusId
     */
    int deleteByPrimaryKey(Integer syllabusId);

    /** 
     * 插入数据库记录
     * @param record
     */
    int insert(SyllabusInfo record);

    /** 
     * 动态字段,写入数据库记录
     * @param record
     */
    int insertSelective(SyllabusInfo record);

    /** 
     * 根据指定的条件查询符合条件的数据库记录
     * @param example
     */
    List<SyllabusInfo> selectByExample(SyllabusInfoCriteria example);

    /** 
     * 根据指定主键获取一条数据库记录
     * @param syllabusId
     */
    SyllabusInfo selectByPrimaryKey(Integer syllabusId);

    /** 
     * 动态根据指定的条件来更新符合条件的数据库记录
     * @param record
     * @param example
     */
    int updateByExampleSelective(@Param("record") SyllabusInfo record, @Param("example") SyllabusInfoCriteria example);

    /** 
     * 根据指定的条件来更新符合条件的数据库记录
     * @param record
     * @param example
     */
    int updateByExample(@Param("record") SyllabusInfo record, @Param("example") SyllabusInfoCriteria example);

    /** 
     * 动态字段,根据主键来更新符合条件的数据库记录
     * @param record
     */
    int updateByPrimaryKeySelective(SyllabusInfo record);

    /** 
     * 根据主键来更新符合条件的数据库记录
     * @param record
     */
    int updateByPrimaryKey(SyllabusInfo record);
}