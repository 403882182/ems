
package com.jyw.marketing.mapper;

import com.jyw.model.TemplateInfo;
import com.jyw.model.TemplateInfoCriteria;
import java.util.List;
import org.apache.ibatis.annotations.Param;

public interface TemplateInfoMapper {
    /** 
     * 根据指定的条件获取数据库记录数
     * @param example
     */
    long countByExample(TemplateInfoCriteria example);

    /** 
     * 根据指定的条件删除数据库符合条件的记录
     * @param example
     */
    int deleteByExample(TemplateInfoCriteria example);

    /** 
     * 根据主键删除数据库的记录
     * @param templateId
     */
    int deleteByPrimaryKey(Integer templateId);

    /** 
     * 插入数据库记录
     * @param record
     */
    int insert(TemplateInfo record);

    /** 
     * 动态字段,写入数据库记录
     * @param record
     */
    int insertSelective(TemplateInfo record);

    /** 
     * 根据指定的条件查询符合条件的数据库记录
     * @param example
     */
    List<TemplateInfo> selectByExample(TemplateInfoCriteria example);

    /** 
     * 根据指定主键获取一条数据库记录
     * @param templateId
     */
    TemplateInfo selectByPrimaryKey(Integer templateId);

    /** 
     * 动态根据指定的条件来更新符合条件的数据库记录
     * @param record
     * @param example
     */
    int updateByExampleSelective(@Param("record") TemplateInfo record, @Param("example") TemplateInfoCriteria example);

    /** 
     * 根据指定的条件来更新符合条件的数据库记录
     * @param record
     * @param example
     */
    int updateByExample(@Param("record") TemplateInfo record, @Param("example") TemplateInfoCriteria example);

    /** 
     * 动态字段,根据主键来更新符合条件的数据库记录
     * @param record
     */
    int updateByPrimaryKeySelective(TemplateInfo record);

    /** 
     * 根据主键来更新符合条件的数据库记录
     * @param record
     */
    int updateByPrimaryKey(TemplateInfo record);
}